...
int main()
{
    ...
    for(int i=1;i<=n;i++)
    {
        if(dis2(C.first,P[i])>C.second+EPS)
        {
            C={P[i],0};
            for(int j=1;j<i;j++)
                if(dis2(C.first,P[j])>C.second+EPS)
                {
                    C={(P[i]+P[j])/2,dis2(P[i],P[j])/4};
                    for(int k=1;k<j;k++)
                        if(dis2(C.first,P[k])>C.second+EPS)
                            C=get_circle(P[i],P[j],P[k]);
                }
        }
    }
    cout<<fixed<<setprecision(2)<<C.first.x<<" "<<C.first.y<<" "<<sqrt(C.second)<<endl;
}