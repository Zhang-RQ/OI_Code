...
int main()
{
	read(n,m);
	int mx=m;
	for(int i=1,x;i<=n;i++) read(x),c[x]++;
	for(int i=1;i<=mx;i++) c[i]=dec(0,4*c[i]);
	c[0]=1;poly_sqrt(c,h,mx+1);
	h[0]++;poly_inverse(h,f,mx+1);
	for(int i=1;i<=mx;i++) f[i]=f[i]*2%P;
