...
void solve()
{
	...
	if(k==1)
	{
		ll Ans=0;
		for(int i=0;i<=n-2;i++)
			Inc(Ans,C(n,i)*ksm(n,n-2-i)%P*ifac[n-2-i]%P);
		printf("%lld\n",fac[n-2]*Ans%P*ksm(ksm(n,n-2),P-2)%P);
		return;
	}
	Poly F;
	F.resize(n);
	for(int i=0;i<n-1;i++)
		F[i]=ksm(i+1,k)*ifac[i]%P;
	F=Pow(F,n);
	printf("%lld\n",fac[n-2]*F[n-2]%P*ksm(ksm(n,n-2),P-2)%P);
}
